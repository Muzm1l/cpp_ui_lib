...
void BDWGraph::drawZeroAxis()
{
    ...
    // X depends on the current Y range (customYMin/Max), Y endpoints are clamped
    // to drawingArea; the timestamp passed to mapDataToScreen only feeds the
    // y-mapping, which we then overwrite. Cheap regardless.
    ...
    // First-time create (or scene was swapped out from under us).
    if (m_zeroAxisLineItem) {
        delete m_zeroAxisLineItem;
        m_zeroAxisLineItem = nullptr;
    }
...
void BDWGraph::refreshOverlaysAfterVisibleTimeRangeChange()
{
    // Called by drawIncremental() in the RANGE_UPDATE_ONLY branch (the path
    // hit by ZoomPanel live drag via setCustomYRangeLive -> YRangeChange).
    drawZeroAxis();
}
